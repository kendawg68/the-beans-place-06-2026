...
function ImageRow( { images, offset = 0 }) {
    ...
    return(
        <div className="carousel-row" style={{ transform: `translated(${offset}px, 0,0)`}}>
...
export default function FeaturesSection() {
    ...
    useEffect( () => {
        const handleScroll = () => {
            ...
            // Progress: 0 when station enters bottom, 1 when it leaves top
            const progress = 1 - rect.bottom / (view + rect.height);
            const p = Math.max(0, Math.min(0, Math.min(1, progress)));

            // Each row moves at differnt speeds/directions based on scroll progress
            ...
            setOffsets([
                ...
                -p * range + 0.7 // row 3 slides left slower111
            ]);
        };
